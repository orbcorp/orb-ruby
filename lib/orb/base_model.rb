...
module Orb
  ...
  module Converter
    ...
    def dump(value)
      case value
      in Array
        value.map { Orb::Unknown.dump(_1) }
      in Hash
        value.transform_values { Orb::Unknown.dump(_1) }
      in Orb::BaseModel
    ...
    class << self
      ...
      def dump(target, value)
        target.is_a?(Orb::Converter) ? target.dump(value) : Orb::Unknown.dump(value)
      end
    end
  end
  ...
  module Enum
    ...
    # @return [Boolean]
    def ==(other)
      other.is_a?(Module) && other.singleton_class <= Orb::Enum && other.values.to_set == values.to_set
    end
  ...
  module Union
    ...
    private def variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Orb::Converter.type_info(spec)]
        in Proc | Orb::Converter | Class | Hash
          [nil, Orb::Converter.type_info(key)]
        end
    ...
    def coerce(value, state:)
      if (target = resolve_variant(value))
        return Orb::Converter.coerce(target, value, state: state)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        coerced = Orb::Converter.coerce(target, value, state: state)
    ...
    def dump(value)
      if (target = resolve_variant(value))
        return Orb::Converter.dump(target, value)
      end

      known_variants.each do
        target = _2.call
        return Orb::Converter.dump(target, value) if target === value
      end
  ...
  class ArrayOf
      ...
      value
        .map do |item|
          ...
          in [true, nil]
            ...
            Orb::Converter.coerce(target, item, state: state)
          end
        end
    end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Array) ? value.map { Orb::Converter.dump(target, _1) } : super
    end
  ...
  class HashOf
    ...
    #
    # @return [Boolean]
    def ==(other) = other.is_a?(Orb::HashOf) && other.nilable? == nilable? && other.item_type == item_type
        ...
        .to_h do |key, val|
          ...
          v =
            ...
            in [true, nil]
              ...
              Orb::Converter.coerce(target, val, state: state)
            end
    ...
    def dump(value)
      target = item_type
      value.is_a?(Hash) ? value.transform_values { Orb::Converter.dump(target, _1) } : super
    end
  ...
  class BaseModel
    ...
    class << self
      ...
      private def add_field(name_sym, required:, type_info:, spec:)
        type_fn, info =
          case type_info
          in Proc | Orb::Converter | Class
            [Orb::Converter.type_info({**spec, union: type_info}), spec]
          in Hash
            [Orb::Converter.type_info(type_info), type_info]
          end

        setter = "#{name_sym}="
        api_name = info.fetch(:api_name, name_sym)
        nilable = info[:nil?]
        const = required && !nilable ? info.fetch(:const, Orb::Util::OMIT) : Orb::Util::OMIT
        ...
        define_method(name_sym) do
          target = type_fn.call
          value = @data.fetch(name_sym) { const == Orb::Util::OMIT ? nil : const }
          state = {strictness: :strong, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
          (nilable || !required) && value.nil? ? nil : Orb::Converter.coerce(target, value, state: state)
          ...
          raise Orb::ConversionError.new(message)
        end
      end
      ...
      #
      # @return [Boolean]
      def ==(other) = other.is_a?(Class) && other <= Orb::BaseModel && other.fields == fields
    end
    ...
    class << self
      ...
      def coerce(value, state:)
        ...
        unless (val = Orb::Util.coerce_hash(value)).is_a?(Hash)
          exactness[:no] += 1
          return value
        end
        exactness[:yes] += 1
        ...
        fields.each do |name, field|
          ...
          converted =
            if item.nil? && (nilable || !required)
              ...
              coerced = Orb::Converter.coerce(target, item, state: state)
              case target
              in Orb::Converter | Symbol
                coerced
              else
                item
              end
      ...
      def dump(value)
        ...
        coerced.each do |key, val|
          ...
          in nil
            ...
            in :coerce
              ...
              target = type_fn.call
              acc.store(api_name, Orb::Converter.dump(target, val))
            end
          end
        end
        ...
        known_fields.each_value do |field|
          ...
          next if mode == :coerce || acc.key?(api_name) || const == Orb::Util::OMIT
