...
class Orb::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      Orb::Internal::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      Orb::Internal::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      Orb::Internal::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      Orb::Internal::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      Orb::Internal::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      Orb::Internal::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = Orb::Internal::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: Orb::Internal::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      Orb::Internal::Util.dig(1, nil) => 1
      Orb::Internal::Util.dig({a: 1}, :b) => nil
      Orb::Internal::Util.dig({a: 1}, :a) => 1
      Orb::Internal::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      Orb::Internal::Util.dig([], 1) => nil
      Orb::Internal::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      Orb::Internal::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
...
class Orb::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      ...
      parsed = Orb::Internal::Util.parse_uri(url)
      unparsed = Orb::Internal::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, Orb::Internal::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        Orb::Internal::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        Orb::Internal::Util.join_parsed_uri(
          Orb::Internal::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        Orb::Internal::Util.join_parsed_uri(
          Orb::Internal::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class Orb::Test::RegexMatchTest < Minitest::Test
  ...
  def test_jsonl_content
    ...
    cases.each do |header, verdict|
      assert_pattern do
        Orb::Internal::Util::JSONL_CONTENT.match?(header) => ^verdict
      end
    end
  end
end
...
class Orb::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      ...
      @io = Orb::Internal::Util::ReadIOAdapter.new(encoded.to_enum) {}
  ...
  def test_encoding_length
    headers, = Orb::Internal::Util.encode_content(
...
class Orb::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Orb::Internal::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Orb::Internal::Util.writable_enum do |y|
...
class Orb::Test::UtilFusedEnumTest < Minitest::Test
  def test_rewind_closing
    ...
    fused = Orb::Internal::Util.fused_enum(enum, external: true) do
      touched = true
      loop { enum.next }
    end
  ...
  def test_thread_interrupts
    ...
    fused_1 = Orb::Internal::Util.fused_enum(enum, external: true) { loop { enum.next } }
    fused_2 = Orb::Internal::Util.chain_fused(fused_1) { fused_1.each(&_1) }
    fused_3 = Orb::Internal::Util.chain_fused(fused_2) { fused_2.each(&_1) }
  ...
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Orb::Internal::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = Orb::Internal::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Orb::Internal::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Orb::Internal::Util.fused_enum(enum)
    fused_2 = Orb::Internal::Util.decode_lines(fused_1)
    fused_3 = Orb::Internal::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Orb::Internal::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end

class Orb::Test::UtilContentDecodingTest < Minitest::Test
  def test_charset
    cases = {
    ...
    cases.each do |content_type, encoding|
      Orb::Internal::Util.force_charset!(content_type, text: text)
...
class Orb::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Orb::Internal::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Orb::Internal::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Orb::Internal::Util.decode_sse(input).map(&:compact)
